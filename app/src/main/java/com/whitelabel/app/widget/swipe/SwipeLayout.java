...
public class SwipeLayout extends FrameLayout {
    ...
    private boolean closeOther= true;
    public void setSwichCloseOther(boolean switchs) {
        closeOther=switchs;
    }
    //这是管理swipe的,例如打开一个关闭其他所有
    private SwipeLayoutCallBack swipeLayoutCallBack;
    public void setCallBack(SwipeLayoutCallBack swipeLayoutCallBack2){
        //swipeAdapter, not change this callBack
        swipeLayoutCallBack=swipeLayoutCallBack2;
    }
    //这是在recycleView里监控状态,改变UI用的
    private SwipeLayoutCallBack statuCallBack;
    public void setStatuCallBack(SwipeLayoutCallBack statuCallBack2){
        statuCallBack=statuCallBack2;
    }
    ...
    public void open(boolean smooth, boolean notify) {
        ...
        if(closeOther&&swipeLayoutCallBack!=null) {
            swipeLayoutCallBack.swipeCallback(this,SwipeLayoutCallBack.type_reall);
        }
        //save the open swipeLayout
        if(swipeLayoutCallBack!=null) {
            swipeLayoutCallBack.swipeCallback(this,SwipeLayoutCallBack.type_add);
        }
        //statuListener
        if(statuCallBack!=null){
            statuCallBack.swipeCallback(this,SwipeLayoutCallBack.type_add);
        }
    ...
    public void close(boolean smooth, boolean notify) {
        ...
        if(statuCallBack!=null) {
            statuCallBack.swipeCallback(this,SwipeLayoutCallBack.type_re);
        }
        invalidate();
    }
    public void close(boolean smooth, boolean notify,boolean callback) {
        if(callback) {
            // if  click del ,not call def swipeCallback,else Thread error.
            swipeLayoutCallBack.swipeCallback(this,SwipeLayoutCallBack.type_re);
        }
        if(statuCallBack!=null){
            statuCallBack.swipeCallback(this,SwipeLayoutCallBack.type_re);
        }
