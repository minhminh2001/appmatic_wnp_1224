package com.whitelabel.app.widget.circularProgressView;
...
//            color = t.data;
            color= WhiteLabelApplication.getAppConfiguration().getThemeConfig().getPrimaryColor();
        }
    ...
    public void setProgress(final float currentProgress) {
        ...
        if (!isIndeterminate) {
            ...
            progressAnimator.addUpdateListener(new CircularAnimationListener(this,1));
            progressAnimator.addListener(new MeAnimatorListenerAdapter(CircularProgressView.this,currentProgress));
    ...
    static  class MeAnimatorListenerAdapter extends AnimatorListenerAdapter{
        float mCurrentProgress=0;
        WeakReference<CircularProgressView> mView;
        public MeAnimatorListenerAdapter(CircularProgressView  circularProgressView,float currentProgress){
            mView=new WeakReference<CircularProgressView>(circularProgressView);
        ...
        @Override
            ...
            for(CircularProgressViewListener listener : mView.get().listeners) {
                 listener.onProgressUpdateEnd(mCurrentProgress);
            }
        }
    ...
    static class  CircularAnimationListener implements   ValueAnimator.AnimatorUpdateListener{
        WeakReference<CircularProgressView> mView;
        int mType;
        public CircularAnimationListener(CircularProgressView view,int type){
            mView=new WeakReference<CircularProgressView>(view);
            mType=type;
        }
        @Override
        public void onAnimationUpdate(ValueAnimator animation) {
            if(mView.get()==null)return;
            if(mType==1) {
                mView.get().actualProgress = (Float) animation.getAnimatedValue();
            }else if(mType==2){
                mView.get().startAngle = (Float) animation.getAnimatedValue();
            }else if(mType==3){
                mView.get().indeterminateSweep = (Float) animation.getAnimatedValue();
            }else if(mType==4){
                mView.get(). indeterminateRotateOffset= (Float) animation.getAnimatedValue();
            }
            mView.get().invalidate();
        }
    }

    static class  CircularAnimationListener1 implements   ValueAnimator.AnimatorUpdateListener{
        WeakReference<CircularProgressView> mView;
        float mMaxSweep;
        float mStart;
        public CircularAnimationListener1(CircularProgressView view,float maxSweep,float start){
            mView=new WeakReference<CircularProgressView>(view);
            mMaxSweep=maxSweep;
            mStart=start;
        }
        ...
        public void onAnimationUpdate(ValueAnimator animation) {
            if(mView.get()==null)return;
            mView.get().startAngle = (Float) animation.getAnimatedValue();
            mView.get().indeterminateSweep = mMaxSweep -  mView.get().startAngle + mStart;
            mView.get().invalidate();
        }
    }
    ...
    public void resetAnimation() {
        ...
        if(!isIndeterminate)
        {
            ...
            startAngleRotate.addUpdateListener(new CircularAnimationListener(this,2));
        ...
        else
        {
            ...
            indeterminateAnimator.addListener(new AnimatorListenerAdapter1(this));
  ...
  static   class AnimatorListenerAdapter1 extends AnimatorListenerAdapter{
        private WeakReference<CircularProgressView> mView;
      public AnimatorListenerAdapter1(CircularProgressView view){
    ...
    private AnimatorSet createIndeterminateAnimator(float step)
    {
        ...
        frontEndExtend.addUpdateListener(new CircularAnimationListener(this,3));
        ...
        rotateAnimator1.addUpdateListener(new CircularAnimationListener(this,4));
        ...
        backEndRetract.addUpdateListener(new CircularAnimationListener1(this,maxSweep,start));
        ...
        rotateAnimator2.addUpdateListener(new CircularAnimationListener(this,4));
