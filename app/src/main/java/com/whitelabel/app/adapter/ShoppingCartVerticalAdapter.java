...
public class ShoppingCartVerticalAdapter extends SwipeableAdapter {
    ...
    private final View.OnClickListener subtractListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            ShoppingCartListEntityCell shoppingCart = (ShoppingCartListEntityCell) list
                .get(position);
        ...
        public void onClick(View v) {
            ...
            if (!WhiteLabelApplication.getAppConfiguration().isSignIn(context)) {
                ...
                if ("1".equals(shoppingCart.getInStock())) {
                    ...
                    JLogUtils.d(TAG,
                        "shoppingCart--newCount=" + newCount + "--------------curretnQty=" +
                            mCurrentStockQty + "-------------stockQty=" + stockQty);
                    if (newCount <= mCurrentStockQty) {
                        shoppingCart.setQty(newCount + "");
                        sendRequestToChangeCount(position, shoppingCart.getId(), newCount,
                            "opt-plus");
                    } else {
                        JViewUtils.showMaterialDialog(context, null,
                            context.getResources().getString(R.string.insufficient_stock), null);
                    }
                } else {
                    JViewUtils.showMaterialDialog(context, null,
                        context.getResources().getString(R.string.insufficient_stock), null);
    ...
    public static List<TMPLocalCartRepositoryProductEntity> shoppingCarToTMPLocal(
        ArrayList<ShoppingCartListBase> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (hashs != null) {
                for (int j = 0; j < hashs.size(); j++) {
                    TMPLocalCartRepositoryProductOptionEntity option = new
                        TMPLocalCartRepositoryProductOptionEntity();
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == 1) {
            ...
            View view = LayoutInflater.from(context)
                .inflate(R.layout.activity_shopping_cart_vertical_cell, null);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        ...
        if ("1".equals(sc.getInStock())) {
            ...
            if ("1".equals(sc.getHasError())) {
                ...
                viewHolder.ivShoppingCartIcon
                    .setBackgroundResource(R.mipmap.ic_checkout_error_mark_item);
                ((ViewHolder) holder).tvShoppingCartErrorMsg
                    .setTextColor(ContextCompat.getColor(context, R.color.red_cart_shopping_error));
        ...
        if (viewHolder.tvOutOfStock.getVisibility() == View.VISIBLE || viewHolder.unavailable
        ...
        try {
            if (Double.parseDouble(sc.getPrice()) == Double.parseDouble(sc.getFinalPrice())) {
                ...
                viewHolder.tvPrice.setText(WhiteLabelApplication.getAppConfiguration().getCurrency()
                    .getName() + " " + JDataUtils.formatDouble(sc.getPrice()));
            }
            viewHolder.tvFinalPrice.setText(
                WhiteLabelApplication.getAppConfiguration().getCurrency()
                    .getName() + " " + JDataUtils.formatDouble(sc.getFinalPrice()));
        } catch (Exception ex) {
            ex.getStackTrace();
        }
        ...
        if (!TextUtils.isEmpty(sc.getVendorDisplayName())) {
            ...
            String soldBy = viewHolder.tvCheckMername.getContext().getResources()
        ...
        if (productAttributes != null && productAttributes.size() > 0) {
            ...
            if (productAttributes.size() > 0) {
                ...
                attributeStr = new StringBuilder(
                    attributeStr.substring(0, attributeStr.length() - 2));
        ...
        try {
            if (!sc.getImage().equals(viewHolder.imageView.getTag())) {
                ...
                JImageUtils
                    .downloadImageFromServerByUrl(context, mImageLoader, viewHolder.imageView,
                        sc.getImage(), JToolUtils.dip2px(context, 150),
                        JToolUtils.dip2px(context, 150));
            }
        } catch (Exception ex) {
...
//            GaTrackHelper.getInstance().googleAnalyticsDeleteCart(context, productId, name);
    ...
    private void sendRequestToChangeCount(final int position, String cellId, final int newCount,
        ...
        mShoppingCartDao.requestChangeCount(
            WhiteLabelApplication.getAppConfiguration().getUserInfo(context).getSessionKey(),
            cellId, newCount + "", bean);

    }

    private void sendRequestToDeteleteCell(final int position, String itemId) {
        mDialog = JViewUtils.showProgressDialog(context);
        mShoppingCartDao.deleteProductFromShoppingCart(
            WhiteLabelApplication.getAppConfiguration().getUserInfo(context).getSessionKey(),
            itemId, position + "");
    }
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View view) {
            ...
            rlShoppingCartCellContent = (RelativeLayout) view
                .findViewById(R.id.rl_shopping_cart_cell_content);
            ivShoppingCartDelete = (ImageView) view.findViewById(R.id.iv_shopping_cart_delete);
            llShoppingcartCellPoint = (LinearLayout) view
                .findViewById(R.id.ll_shoppingcart_cell_point);
            tvShoppingcartCellPoint1 = (TextView) view
                .findViewById(R.id.tv_shoppingcart_cell_point1);
            tvShoppingcartCellPoint2 = (TextView) view
                .findViewById(R.id.tv_shoppingcart_cell_point2);
            tvShoppingcartCellPoint3 = (TextView) view
                .findViewById(R.id.tv_shoppingcart_cell_point3);
            tvShoppingcartSplit = (TextView) view.findViewById(R.id.tv_shoppingcart_split);
            rlShoppingcartCountSub = (RelativeLayout) view
                .findViewById(R.id.rl_shoppingcart_count_sub);
            rlShoppingcartCountPlus = (RelativeLayout) view
                .findViewById(R.id.rl_shoppingcart_count_plus);
    ...
    private static final class DataHandler extends Handler {
        ...
        public DataHandler(Context context,
            ShoppingCartVerticalAdapter shoppingCartAdapterCallback) {
            ...
            switch (msg.what) {
                case ShoppingCarDao.REQUEST_CHANGESHOPPINGCARCOUNT:
                    if (msg.arg1 == ShoppingCarDao.RESPONSE_SUCCESS) {
                        final ShoppingCartDeleteCellEntity shoppingCartUpdateCellCount =
                            (ShoppingCartDeleteCellEntity) msg.obj;
                        //success
                        if (shoppingCartUpdateCellCount != null && shoppingCartUpdateCellCount
                            .getStatus() == 1) {
                            int position = Integer
                                .parseInt(shoppingCartUpdateCellCount.getParams().getKey());
                            try {
                                ShoppingCartListEntityCell shoppingCart =
                                    (ShoppingCartListEntityCell) mAdapter
...
//                                        shoppingCart.setInStock("0");
                                        JViewUtils.showMaterialDialog(mContext.get(), null,
                                            mContext.get().getString(R.string.insufficient_stock),
                                            null);
                                    }
                                }
                                mAdapter.get().callback
                                    .updateShoppingData(shoppingCartUpdateCellCount);
                                //TODO josyon not now to refresh, to call list api and refresh
                                if (mAdapter.get().iShoppingCartAddOrSubtractCallback != null) {
                                    mAdapter.get().iShoppingCartAddOrSubtractCallback
                                        .callRefreshData();
                                }
//                                mAdapter.get().notifyDataSetChanged();
                        ...
                        if (!JToolUtils
                            .expireHandler((Activity) mContext.get(), errorMsg.getErrorMessage(),
                                2000)) {
                            try {
                                ShoppingCartListEntityCell shoppingCart =
                                    (ShoppingCartListEntityCell) mAdapter
                ...
                case ShoppingCarDao.REQUEST_ERROR:
                    ...
                    if (msg.arg1 == ShoppingCarDao.REQUEST_CHANGESHOPPINGCARCOUNT) {
                        ...
                        int position = Integer.parseInt(keyValueBean.getKey());
                        String option = keyValueBean.getValue();
                        ShoppingCartListEntityCell shoppingCart = (ShoppingCartListEntityCell)
                            mAdapter
                                .get().list.get(position);
                        int qty = Integer.parseInt(shoppingCart.getQty());
                        if ("opt-plus".equals(option)) {
                ...
                case ShoppingCarDao.REQUEST_DELETEFROMSHOPPINGCART:
                    mAdapter.get().closeDialog();
                    if (msg.arg1 == ShoppingCarDao.RESPONSE_SUCCESS) {
                        final ShoppingCartDeleteCellEntity shoppingCartDeleteCell =
                            (ShoppingCartDeleteCellEntity) msg.obj;
                        ...
                        if (shoppingCartDeleteCell.getStatus() == 1) {//success
                            ...
                            try {
                                ...
                                cell = (ShoppingCartListEntityCell) mAdapter.get().list
                                    .get(position);
                                //TODO joyson may be use
//                                GaTrackHelper.getInstance().googleAnalyticsDeleteCart(mContext
// .get(), cell.getProductId(), cell.getName());
                            } catch (Exception ex) {
                                ex.getStackTrace();
                            }
                            try {
                                ...
                                mAdapter.get().callback
                                    .deleteShoppingData(shoppingCartDeleteCell, position);
...
//                                }
                        ...
                        if (!JToolUtils
                            .expireHandler((Activity) mContext.get(), msgBean.getErrorMessage(),
                                2000)) {
                            Toast.makeText(mContext.get(), msgBean.getErrorMessage() + "",
