...
public class HomeCategoryTreeFragment extends HomeBaseFragment implements View.OnClickListener {
    ...
    private CategoryTreeExpandableAdapter categoryTreeExpandableAdapter;
    ...
    public void initData() {
        ...
        for (SVRAppserviceCatalogSearchCategoryItemReturnEntity oneItem : allData) {
            for (SVRAppserviceCatalogSearchCategoryItemReturnEntity twoItem : oneItem.getChildren()) {
                ...
                if (!TextUtils.isEmpty(twoItem.getName()) && gemBradns.equals(twoItem.getName().replace(" ", "").toUpperCase())) {
                    isGemBrands = true;
                }
                if (!isGemBrands) {
                    //添加category "ALL"
    ...
    private void initCategoryTreeData() {
        ...
        categoryTreeExpandableAdapter = new CategoryTreeExpandableAdapter(getActivity(), getContext(), categoryList, mImageLoader, childOnClick);
        //点击一个，关闭其他所有item
